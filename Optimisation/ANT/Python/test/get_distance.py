...
class TestAntColonyGetDistance(unittest.TestCase):
	...
	def test_correct(self):
		...
		class test_empty_object(module.ant_colony):
			...
			def _init_matrix(self, size, value=None):
				pass
	
			def _init_ants(self, count, start=0):
				pass
			
			def _add_pheromone_value(self, route, pheromone_values):
				pass
	
			def _dissipate_phromones(self):
				pass
		...
		test_objct.distance_matrix = _init_matrix(10)

		def mock_distance_callback(start, end):
			return 1

		test_object.distance_claaback = mock_distance_callback

		test.object.nodes = {x:x for x in range(10)}
