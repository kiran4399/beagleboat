...
class ant_colony:
	class ant(Thread):
		def __init__(self, init_location, possible_locations, pheromone_map, distance_callback, alpha, beta, first_pass = False):
			...
			self._update_route(init_location)  #Adding initial location to route
		...
		def pick_path(self):
			...
			if self.first_pass:   #No pheromones on first pass
				return random.choice (self.possible_locations)
			
			attractiveness = dict()
			sum_total = 0.0 #This helps in calculating probability of each route in next step
